...
/// Minimal URLSession replacement for unit tests.
final class MockSession: URLSession {
    var handler: (URLRequest) throws -> (Data, URLResponse) = { _ in
        (Data(), HTTPURLResponse(url: URL(string: "http://x")!, statusCode: 200, httpVersion: nil, headerFields: nil)!)
    }
